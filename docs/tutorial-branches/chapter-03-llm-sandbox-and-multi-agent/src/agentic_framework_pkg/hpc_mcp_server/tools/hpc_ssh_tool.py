...
def _get_hpc_connection(
    ...
    # In Docker, this will be mounted at /home/appuser/.ssh/hpc_key
    ...
    # Fallback to password (not recommended)
        ...
        raise ValueError(
            "No authentication method configured. Set HPC_SSH_KEY_PATH (mounted key in container) or HPC_SSH_PASSWORD"
        )
...
def register_tools(mcp: FastMCP):
    ...
    async def test_hpc_connection(
        ...
        """
        Test SSH connection to a REMOTE HPC cluster via SSH.

        Use this tool to verify connectivity to an external HPC cluster that you access via SSH.
        This establishes an SSH connection and runs basic commands to verify the cluster is reachable.

        NOTE: This connects to a REMOTE cluster via SSH, not the local Nextflow/BLAST container.
        For running Nextflow/BLAST locally in this container, use run_nextflow_blast_pipeline instead.
    ...
    async def submit_slurm_job(
        ...
        """
        Submit a Slurm batch job to a REMOTE HPC cluster via SSH.

        Use this tool to submit jobs to an external HPC cluster that you access via SSH.
        The script must already exist on the remote HPC cluster's filesystem.

        NOTE: This submits jobs to a REMOTE cluster via SSH, not the local Nextflow container.
        For running Nextflow/BLAST locally, use run_nextflow_blast_pipeline instead.
    ...
    async def check_slurm_job_status(
        ...
        """
        Check status of a Slurm job on a REMOTE HPC cluster via SSH.

        Use this tool to monitor jobs on an external HPC cluster that you access via SSH.
        Queries the Slurm scheduler for current job state.

        NOTE: This checks jobs on a REMOTE cluster via SSH, not local processes.
