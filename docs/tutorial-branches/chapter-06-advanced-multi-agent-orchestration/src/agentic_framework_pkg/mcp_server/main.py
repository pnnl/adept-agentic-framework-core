"""
Main entrypoint for the default MCP (Multi-Component Protocol) server.

This module is responsible for initializing the FastAPI application, setting up
necessary middleware (like request logging), and registering all the tool modules
that this server will expose. It orchestrates the startup process, including
database initialization and the configuration of the Uvicorn server.

The tool registration is handled by the `setup_mcp_server` function, which
imports and calls the `register_tools` function from each relevant tool module
(e.g., `general_tools`, `csv_rag_tool`, `mcp_tool_proxy`). This modular
approach allows for easy addition or removal of toolsets.
"""
...
def setup_mcp_server(mcp_instance: mcp = mcp) -> FastAPI:
    ...
    Tools are registered explicitly for pedagogical clarity.
    For auto-discovery (§2.5 step 4), see plugin_loader.py which can
    replace the explicit calls below with a single scan of the tools/ directory.
    """
    logger.info("Setting up MCP server and registering tools...")
    general_tools.register_tools(mcp)
    csv_rag_tool.register_tools(mcp, llm_agnostic_client_instance)
    uniprot_tool.register_tools(mcp)
    websearch_tool.register_tools(mcp)
    blastq_tool.register_tools(mcp)
    multi_agent_tool.register_tools(mcp, llm_agnostic_client_instance)
    pubchem_tool.register_tools(mcp)
    alphafold_tool.register_tools(mcp, llm_agnostic_client_instance)
